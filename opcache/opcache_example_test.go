...
func ExampleOpCache() {
	...
	var getPointCache = opcache.NewOpCache[structs.Struct2[int, int], *Point](opcache.OpCacheConfig{ResultExpiration: 100 * time.Millisecond})
	...
	GetPointFast := func(x, y int) (*Point, error) {
		return getPointCache.Get(
			structs.Struct2Of(x, y), // Key constructed from all arguments
...
func ExampleOpCache_multi_return() {
	...
	var getPointCache = opcache.NewOpCache[structs.Struct2[int, int], structs.Struct2[*Point, int]](opcache.OpCacheConfig{ResultExpiration: 100 * time.Millisecond})

	// Function to use which utilizes getPointCache (has identical signature to that of GetPoint):
	GetPointFast := func(x, y int) (*Point, int, error) {
		mr, err := getPointCache.Get(
			structs.Struct2Of(x, y), // Key constructed from all arguments
			func() (structs.Struct2[*Point, int], error) {
				p, n, err := GetPoint(x, y)
				return structs.Struct2Of(p, n), err // packing multiple results
			},
		)
...
func ExampleOpCache_MultiGet() {
	...
	var calcCache = opcache.NewOpCache[int, CalcResult](opcache.OpCacheConfig{
...
func ExampleOpCache_MultiGet_multi_inputargs() {
	...
	var calcCache = opcache.NewOpCache[structs.Struct2[int, int], CalcResult](opcache.OpCacheConfig{
		ResultExpiration:      100 * time.Millisecond,
		ResultGraceExpiration: 50 * time.Millisecond,
	})
	...
	MultiCalcFast := func(xs, ys []int) ([]CalcResult, []error) {
		keys := make([]structs.Struct2[int, int], len(xs))
		for i, x := range xs {
			keys[i] = structs.Struct2Of(x, ys[i]) // Key constructed from all arguments
		}
		return calcCache.MultiGet(
			keys,
			func(keyIndices []int) ([]CalcResult, []error) {
